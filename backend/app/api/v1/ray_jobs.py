...
from app.services.ray_job_service import (
    ray_job_service, 
    GitHubJobRequest, 
    JobStatus
)
...
async def submit_github_ray_job(
    ...
    """提交基于GitHub的Ray作业"""
    try:
        logger.info(f"User {current_user.id} submitting GitHub Ray job: {job_request.github_repo}")
...
async def get_job_templates(
    ...
    templates = {
        "custom": {
            "name": "Custom Script",
            "description": "Execute any Python script with process_data() function",
            "required_function": "process_data(input_data, config)",
...
async def validate_github_repository(
    ...
    try:
        ...
        try:
            # 验证仓库结构
            validation = await github_service.validate_repository_structure(repo_path, entry_point)

...
def compute_prime_count(start: int, end: int) -> Dict[str, Any]:
    ...
    log_info(f"Computing primes from {start} to {end}")
    primes = []
...
def parallel_sum(numbers: list) -> float:
    """并行计算数组和"""
    log_info(f"Computing sum of {len(numbers)} numbers")
...
def compute_primes_core(
    start_range: int, end_range: int, chunk_size: int
) -> Dict[str, Any]:
    """核心素数计算逻辑 - 可在本地调试"""
    log_info(
        "Starting prime computation",
        start=start_range,
        end=end_range,
...
def compute_fibonacci_core(n: int) -> Dict[str, Any]:
    ...
    final_result = {"fibonacci_n": n, "result": result}
